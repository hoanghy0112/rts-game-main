...
func _check_troop_scene_and_exports(failures: Array[String]) -> void:
	...
	_expect(_approx(float(troop.get("combat_spear_range_m")), 2.47, 0.001), "combat spear range should default to the 1.3x spear reach", failures)
	...
	_expect(_approx(float(troop.get("combat_socket_radius")), 2.275, 0.001), "combat socket radius should default inside the 1.3x spear reach", failures)
	...
	if first_soldier:
		...
		_expect(_approx(float(first_soldier.get("formation_walk_animation_scale")), 0.6666667, 0.001), "troop soldier walk animation should default to 1.5x slower playback", failures)
