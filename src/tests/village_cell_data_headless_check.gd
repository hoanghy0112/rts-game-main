...
func _check_village_region_defaults(failures: Array[String]) -> void:
	...
	_expect(int(region.get("house_max_count")) == 18, "VillageRegion house max count default should be moderately denser", failures)
	_expect(is_equal_approx(float(region.get("house_density")), 1.5), "VillageRegion house density default should increase generated houses", failures)
...
func _check_migrated_scene(scene_path: String, failures: Array[String]) -> void:
	...
	_expect(is_equal_approx(float(region.get("house_density")), 1.5), "%s should use denser house density" % scene_path, failures)
